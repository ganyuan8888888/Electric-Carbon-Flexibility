...
from proposed_library_v28 import library
from plot_revision_v12 import panel,legend,COLORS,STYLES
from plot_uncertainty_v26 import DATES,LABELS
OUT=ROOT/'revision_v28/figures';OUT.mkdir(parents=True,exist_ok=True)
def save(fig,stem):
...
def root(date):return ROOT/f'results/robust_v28/{date}'
def case(date):
...
def _complete_inner_diagnostic():
    ...
    exported={}
    for row,date in enumerate(DATES):
        a=json.loads((ROOT/f'revision_v28/iteration_audit/{date}.json').read_text())
