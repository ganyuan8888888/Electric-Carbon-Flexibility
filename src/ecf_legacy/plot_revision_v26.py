...
from plot_uncertainty_v26 import save,OUT,DATES,LABELS
METHODS=['M1_constant','M2_native','M3_native','M4_proposed']

def root(date):return ROOT/f'results/robust_v26/{date}'
def case(date):
...
def temperatures():
    fig,axes=plt.subplots(3,1,figsize=(7.25,5.25),sharex=True,sharey=True,gridspec_kw={'hspace':.025})
    allvalues=[];data={}
    for j,(date,ax) in enumerate(zip(DATES,axes)):
        net,p=case(date)
        ...
        ax.text(.025,.08,f'({chr(97+j)}) {date}',transform=ax.transAxes,fontsize=8.5)
...
def hourly():
    ...
    fig,axes=plt.subplots(3,1,figsize=(6.8,6.2),sharex=True,gridspec_kw={'hspace':.025})
    ...
    for row,(date,ax) in enumerate(zip(DATES,axes)):
        ...
        ax.set_ylim(0,np.max(co2rows)*1.18);right.set_ylim(min(0,np.min(pccrows)*1.05),np.max(pccrows)*1.12)
    ...
    handles=[Patch(fc=c,ec='black',lw=.35) for c in COLORS]+[Line2D([0],[0],c=c,ls=ls,lw=1.0) for c,ls in zip(COLORS,STYLES)]
    axes[0].legend(handles,LABELS+LABELS,ncol=1,loc='upper left',
        fontsize=6.3,framealpha=1,edgecolor='black',fancybox=False,borderpad=.18,
        labelspacing=.11,handlelength=2.2,handletextpad=.45,borderaxespad=.2)
...
def _complete_inner_diagnostic():
    ...
    exported={}
    for row,date in enumerate(DATES):
        a=json.loads((ROOT/f'revision_v26/iteration_audit/{date}.json').read_text())
