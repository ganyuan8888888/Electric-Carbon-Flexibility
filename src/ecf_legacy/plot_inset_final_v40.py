...
import plot_inset_evidence_v40 as style
from plot_inset_evidence_v40 import *
...
def ac_network():
 data,old=ac_data();fig=plt.figure(figsize=(7.25,6.2));gs=fig.add_gridspec(3,2,wspace=.24,hspace=.23);t=(np.arange(48)+.5)/2
 plotdata=[]
 for j,date in enumerate(DATES):
  ...
  left=fig.add_subplot(gs[j,0]);right=fig.add_subplot(gs[j,1])
  ...
  left.text(.02,.96,f'({chr(97+2*j)}) {date}',transform=left.transAxes,va='top',fontsize=8)
  ...
  right.axhline(100,c='black',ls=':',lw=.85);right.set(xlim=(0,24),ylim=(78,114),ylabel='Maximum branch loading (%)');right.set_xticks([0,4,8,12,16,20,24]);right.set_yticks([80,90,100,110]);axes_style(right)
  right.text(.025,.96,f'({chr(98+2*j)}) Thermal-limit verification',transform=right.transAxes,va='top',fontsize=7.7)
  inset=right.inset_axes([.14,.055,.52,.22]);sel=(t>=17)&(t<=22)
  ...
  if j==0:legend(left,ncol=2,loc='lower left',fontsize=6.5)
  if j<2:left.tick_params(labelbottom=False);right.tick_params(labelbottom=False)
  else:left.set_xlabel('Time (h)');right.set_xlabel('Time (h)')
  plotdata.append(dict(date=date,initial=a,verified=b))
 fig.subplots_adjust(left=.085,right=.985,top=.99,bottom=.07);save(fig,'fig_ac_network_v39',plotdata)
...
def ac_carbon():
 ...
 for j,date in enumerate(DATES):
  ...
  ax.text(.02,.96,f'({chr(97+2*j)}) {date}',transform=ax.transAxes,va='top',fontsize=8)
  ...
  work.text(.02,.96,f'({chr(98+2*j)}) AC correction impact',transform=work.transAxes,va='top',fontsize=7.7)
 ...
 fig.subplots_adjust(left=.085,right=.915,top=.99,bottom=.07);save(fig,'fig_ac_carbon_v39',plotdata)

def algorithm():
 rows=[json.loads(p.read_text()) for p in sorted((ROOT/'revision_v39/experiments/final_benchmark').glob('*.json'))];assert len(rows)==36
 fig=plt.figure(figsize=(7.25,6.1));gs=fig.add_gridspec(3,2,wspace=.38,hspace=.20);sizes=[5,10,20,40];pos=np.arange(4)
 ...
 for j,date in enumerate(DATES):
  ...
  ax.text(.025,.96,f'({chr(97+2*j)}) {date}',transform=ax.transAxes,va='top',fontsize=8);work.text(.025,.96,f'({chr(98+2*j)}) Optimization workload',transform=work.transAxes,va='top',fontsize=7.7)
  if j<2:ax.tick_params(labelbottom=False);work.tick_params(labelbottom=False)
 ...
 fig.subplots_adjust(left=.085,right=.92,top=.945,bottom=.075);save(fig,'fig_scaling_v39',dict(rows=rows,time_unit='minutes',formal_repeats=1))
