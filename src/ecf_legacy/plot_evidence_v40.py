...
def region():
 ...
 fig=plt.figure(figsize=(7.25,7.15));gs=fig.add_gridspec(3,2,width_ratios=[1.1,1],wspace=.46,hspace=.35)
 depth_colors=['#00cc00','#ff0000','#0000ff'];depths=[5,7,9]
 for row,date in enumerate(DATES):
  detail_grid=gs[row,0].subgridspec(2,1,height_ratios=[3,1.25],hspace=.55)
  left=fig.add_subplot(detail_grid[0]);right=fig.add_subplot(gs[row,1]);r=next(x for x in coarse if x['date']==date and x['chart']==0);dr=next(x for x in dense if x['date']==date and x['chart']==0)
  ...
  spread=max(.005,np.ptp(bt));left.set_ylim(bt.min()-.08*spread,bt.max()+(.48 if row==0 else .22)*spread)
  ...
  left.set_ylabel('Budget multiplier');left.set_title(f'({chr(97+row*2)}) {date}',loc='left',fontsize=8,pad=4)
  if row==0:region_handles,region_labels=left.get_legend_handles_labels()
  # Fixed coordinate window, identical rule for each day; show stepwise certification gap.
  inset=fig.add_subplot(detail_grid[1]);mask=(fine>=.55)&(fine<=.70);inset.plot(fine[mask],bt[mask],'k-',lw=.75)
  ...
  inset.set_title('Local boundary detail',fontsize=6.8,pad=3)
  ...
  right.set_title(f'({chr(98+row*2)}) Interval refinement',loc='left',fontsize=8,pad=4)
  if row==0:chart_handles,chart_labels=right.get_legend_handles_labels()
  left.tick_params(labelbottom=False)
  if row==2:inset.set_xlabel('Chart coordinate',fontsize=8);right.set_xlabel('Bisection depth')
  else:right.tick_params(labelbottom=False)
 fig.legend(region_handles,region_labels,loc='upper center',bbox_to_anchor=(.5,1.01),ncol=4,fontsize=7,framealpha=1,edgecolor='black',fancybox=False)
 fig.legend(chart_handles,chart_labels,loc='upper center',bbox_to_anchor=(.5,.975),ncol=3,fontsize=7,framealpha=1,edgecolor='black',fancybox=False)
 fig.subplots_adjust(left=.10,right=.905,top=.90,bottom=.08)
 save(fig,'fig_certification_v38',{'original_data_version':37,'refinement':coarse,'dense':dense,'inset_coordinate_window':[.55,.70]})

def oos():
 fig=plt.figure(figsize=(7.25,6.65));outer=fig.add_gridspec(3,1,hspace=.30);all_data=[]
 for j,date in enumerate(DATES):
  ...
  ax.set_title(f'({chr(97+j)}) {date}',loc='left',fontsize=8,pad=4)
  if j==0:
   handles=[(Patch(fc=c,ec='black',lw=.4),Line2D([0],[0],c=c,ls=ls,marker=m,mfc='white',ms=3,lw=.9)) for c,ls,m in zip(COLORS,STYLES,MARKS)]
   fig.legend(handles=handles,labels=['M1','M4-N','M4 (proposed)'],handler_map={tuple:HandlerTuple(ndivide=None)},ncol=3,loc='upper center',bbox_to_anchor=(.5,1),fontsize=7.5,framealpha=1,edgecolor='black',fancybox=False)
 ...
 fig.subplots_adjust(left=.105,right=.91,top=.925,bottom=.08)
